...
float** Matrix_allocArrayOfArray(int num_rows, int num_cols){
	float ** m = (float**)malloc(sizeof(float*)*num_rows);
	for(int i=0; i<num_rows; i++) {
		m[i] = (float*)malloc(sizeof(float)*num_cols);
	}
	return m;
}
...
float** Matrix_clone(float** src, int num_rows, int num_cols){
	float** m= Matrix_alloc(num_rows, num_cols);
	for(int i=0; i<num_rows; i++) {
...
float** Matrix_copyTransposed(float** src, int num_rows, int num_cols){
	float** m= Matrix_alloc(num_cols, num_rows);
...
int Matrix_extractBlock(float** dest, float** src, 
	...
	if(block_start_row+block_num_rows <= num_rows || block_start_col+block_num_cols <= num_cols) {
		for(int i=0; i<block_num_rows; i++) {
			for(int j=0; j<block_num_cols; j++) {
				dest[i][j] = src[block_start_row+i][block_start_col+j];
			}
		}
	}else{
		return -1;
	}
	dest = Matrix_allocArrayOfArray(block_num_rows, block_num_cols);
	return 0;
}
